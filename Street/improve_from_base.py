...
class ImprovedDigitsModel(nn.Module):
    def __init__(self, class_num=11):
        ...
        # 获取特征维度
        dummy_input = t.randn(1, 3, 128, 224)
...
DigitsResnet50 = ImprovedDigitsModel
...
class Trainer:
    def __init__(self, val=True):
        ...
        self.optimizer = Adam(self.model.parameters(), lr=4e-4, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
        # 使用AdamW优化器
        # self.optimizer = t.optim.Adam(self.model.parameters(), lr=config.lr, weight_decay=1e-2)

        # self.lr_scheduler = CosineAnnealingWarmRestarts(self.optimizer, T_0=10, T_mult=2, eta_min=0)
        # 使用OneCycleLR
        steps_per_epoch = len(self.train_loader)
        self.lr_scheduler = t.optim.lr_scheduler.OneCycleLR(
            self.optimizer,
            max_lr=config.lr * 10,
