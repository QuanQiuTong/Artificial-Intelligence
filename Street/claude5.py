...
class Trainer:
    def __init__(self, val=True):
        ...
        self.lr_scheduler = OneCycleLR(
            self.optimizer,
            max_lr=config.lr,
            epochs=config.epoches,  # 新增参数
            steps_per_epoch=len(self.train_loader),  # 新增参数
            pct_start=0.1,
            div_factor=25,
...
if __name__ == '__main__':
    ...
    # 生成预测结果
    predicts(trainer.best_checkpoint_path, "claude5_result.csv")
