...
class Config:
    ...
    print_interval = 50
    checkpoints = './checkpoints_all'   # 自己创建一个文件夹用来储存权重
    pretrained = None 
    start_epoch = 0
    epoches = 30
    smooth = 0.1
...
class DigitsDatasetAll(Dataset):
    def __init__(self, size=(128, 256), aug=True):
        super().__init__()
        self.aug = aug
        self.size = size
        self.width = 224
        self.batch_count = 0
        # 合并train和val
        imgs = glob(data_dir['train_data'] + '*.png') + glob(data_dir['val_data'] + '*.png')
        labels = {}
        labels.update(json.load(open(data_dir['train_label'], 'r')))
        labels.update(json.load(open(data_dir['val_label'], 'r')))
        self.imgs = [(img, labels[os.path.split(img)[-1]]) for img in imgs if os.path.split(img)[-1] in labels]
...
class Trainer:
    def __init__(self, val=True, use_all_data=False):
        self.device = t.device('cuda') if t.cuda.is_available() else t.device('cpu')
        if use_all_data:
            self.train_set = DigitsDatasetAll()
        else:
        ...
        self.train_loader = DataLoader(self.train_set, batch_size=config.batch_size, shuffle=True, num_workers=8,
                                       pin_memory=True, persistent_workers=True, drop_last=True, collate_fn=self.train_set.collect_fn)
        if val and not use_all_data:
            self.val_loader = DataLoader(DigitsDataset(mode='val', aug=False), batch_size=config.batch_size, num_workers=8, pin_memory=True, drop_last=False, persistent_workers=True)
        else:
...
if __name__ == '__main__':
    trainer = Trainer(val=False, use_all_data=True)
    trainer.train()
    # acc = trainer.eval()
