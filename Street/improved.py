...
checkpoint_path = './improved_checkpoints'
# find the best checkpoint with highest accuracy
# name be like "improved-efficientnet_b0-epoch-1-acc-15.53.pth"
best_checkpoint = None
""" best_acc = 0.0
if os.path.exists(checkpoint_path):
    checkpoints = os.listdir(checkpoint_path)
    for checkpoint in checkpoints:
        if 'improved-efficientnet_b0' in checkpoint and checkpoint.endswith('.pth'):
            li = checkpoint.split('-')
            # print(li)
    ...
    if best_checkpoint:
        print(f'Best checkpoint found: {best_checkpoint}, acc: {best_acc}')
else:
...
class ImprovedConfig:
    ...
    lr = 1e-3 # 5e-4  # 降低学习率以获得更稳定的训练
    weight_decay = 1e-2  # 增加权重衰减以减少过拟合
    class_num = 11  # 与原始相同，包括空类别
    eval_interval = 1
    checkpoint_interval = 1
    print_interval = 50
    checkpoints = checkpoint_path  # 新的检查点目录
    pretrained = None # (checkpoint_path + '/' + best_checkpoint) if best_checkpoint else None  # 使用最佳检查点进行预训练
    start_epoch = 0
    epochs = 30  # 增加训练轮数
    smooth = 0.1
